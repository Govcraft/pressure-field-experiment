...
We presented pressure-field coordination, a decentralized approach to \acp{mas} that achieves coordination through shared state and local pressure gradients rather than explicit orchestration.
...
\Acp{fm}' zero-shot capabilities eliminate the need for domain-specific action representations; pressure-field coordination eliminates the need for complex multi-agent protocols; together they enable simple \acp{mas}.

These results suggest that for domains with measurable quality signals and locally-decomposable structure, implicit coordination through shared state offers not just a simpler but a more effective alternative to explicit hierarchical control. More broadly, this work demonstrates the bidirectional value of integrating \acp{fm} with classical \ac{mas} coordination theory: stigmergy~\cite{grasse1959stigmergie}, self-organization~\cite{serugendo2005selforganisation}, and organizational paradigms~\cite{horling2004survey} provide principled coordination mechanisms that complement \ac{fm} capabilities, while \acp{fm} provide the universal actor capability that makes these mechanisms practical for open-ended tasks.
