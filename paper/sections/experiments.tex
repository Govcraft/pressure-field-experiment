...
\textbf{Key findings}: Pressure-field coordination outperforms all baselines (\S\ref{sec:main-results}). Temporal decay is critical---disabling it reduces solve rate by 10 percentage points (\S\ref{sec:ablations}). The approach maintains consistent performance from 1 to 4 agents (\S\ref{sec:scaling}).
...
Conversation & 26/270 & 9.6\% & 6.7\%--13.7\% \\
...
\textbf{Pressure-field dominates}: Pressure-field achieves 48.5\% solve rate, roughly 5$\times$ higher than the next-best baseline (conversation at 9.6\%). The effect size is large: Cohen's $h = 1.26$ versus conversation on easy problems, and $h > 1.97$ versus all other baselines.

\textbf{Conversation provides intermediate performance}: The AutoGen-style conversation baseline achieves 9.6\% overall, significantly better than hierarchical ($p < 0.001$) but far below pressure-field. Notably, conversation solves only easy problems (28.9\% on easy, 0\% on medium and hard).
...
Decay proves essential---without it, solve rate drops:
...
\emph{Decay dominates}: configurations with decay achieve higher solve rates. Inhibition shows no detectable effect in this domain, possibly because the 50-tick budget provides sufficient exploration without explicit cooldowns.
...
\midrule
Easy & 86.7\% (78/90) & 28.9\% (26/90) & 4.4\% (4/90) & 1.1\% (1/90) \\
Medium & 43.3\% (39/90) & 0.0\% (0/90) & 0.0\% (0/90) & 0.0\% (0/90) \\
Hard & 15.6\% (14/90) & 0.0\% (0/90) & 0.0\% (0/90) & 0.0\% (0/90) \\
\midrule
\textbf{Total} & \textbf{48.5\%} (131/270) & \textbf{9.6\%} (26/270) & \textbf{1.5\%} (4/270) & \textbf{0.4\%} (1/270) \\
\bottomrule
\end{tabular}
...
\item \textbf{The gap widens with difficulty}: On easy problems, pressure-field leads by 57.8 percentage points over conversation (86.7\% vs 28.9\%). On medium and hard problems, the gap becomes absolute---pressure-field solves problems that no baseline can solve.

\item \textbf{Effect sizes are large}: Cohen's $h$ on easy problems: $h = 1.26$ vs conversation, $h = 1.97$ vs hierarchical, $h = 2.18$ vs sequential/random.
\end{enumerate}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{fig3_effect_sizes.pdf}
\caption{Effect sizes (Cohen's $h$) for pressure-field versus each baseline on easy problems. The dashed line indicates the ``large effect'' threshold ($h = 0.8$). All comparisons exceed this threshold, with effects ranging from $h = 1.26$ (vs conversation) to $h = 2.18$ (vs sequential/random).}
...
\caption{Mean ticks to solution on easy problems (solved cases only). Error bars show standard error. Pressure-field converges fastest (17.8 ticks), followed by conversation (30.4 ticks) and hierarchical (40.0 ticks).}
