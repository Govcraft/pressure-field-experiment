...
pub struct TickResult {
    ...
    pub completion_tokens: u32,
    /// Number of LLM proposal calls actually dispatched this tick.
    ///
    /// Under round-robin dispatch, each high-pressure region is sent to exactly
    /// one patch actor, so this equals the number of high-pressure regions
    /// proposed for - not the number of agents.
    pub llm_calls: usize,
...
impl AsyncKernelBuilder {
    ...
    pub async fn spawn(self, runtime: &mut ActorRuntime) -> ActorHandle {
        ...
        let ema_alpha = self.coordinator.config.pressure_ema_alpha;
        let validation_sensor = self.validation_sensor.clone();

        // Spawn the coordinator (it subscribes to SensorReady)
        ...
        if let Some(sensor) = validation_sensor {
            ...
            for (rid, view) in region_views {
                let region_actor = RegionActor {
                    ...
                    sensor: sensor.clone(),
                    pressure_axes: pressure_axes.clone(),
                    ema_alpha,
                };
                let handle = region_actor.spawn(runtime).await;
    ...
    pub async fn run(
        ...
        if let Some(sensor) = validation_sensor {
            ...
            for (rid, view) in region_views {
                let region_actor = RegionActor {
                    ...
                    sensor: sensor.clone(),
                    pressure_axes: pressure_axes.clone(),
                    ema_alpha: config.pressure_ema_alpha,
                };
                let handle = region_actor.spawn(runtime).await;
                region_actors.insert(rid, handle);
            }
        ...
        KernelResult {
            ...
            prompt_tokens,
            completion_tokens,
            // The artifact's final source is retrieved separately via the
            // SaveArtifact message rather than embedded in the result.
            final_source: None,
            tick_results,
            pressure_history,
        }
